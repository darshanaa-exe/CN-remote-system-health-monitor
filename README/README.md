...
NAME: DARSHANA S     SRN: PES1UG24CS913
NAME: PRASHANT       SRN: PES1UG24CS597
NAME: PRAVEEN        SRN: PES1UG24CS832
...
### Web Dashboard — Single Node (node-1)

![Web Dashboard with 1 Node](1.png)
*Live dashboard with 1 online node (node-1) showing an active MEM ALERT, the node comparison bar chart, and recent packet history with all entries in ALERT state due to memory exceeding the 70% threshold.*

---

### Client Terminal — node-1

![Client Terminal node-1](2.png)
*Terminal output of node-1 running `client.py`, transmitting encrypted UDP packets every 5 seconds. Each `[SENT]` line shows the real-time CPU, memory, and disk readings collected via psutil.*

---

### Web Dashboard — Two Nodes (node-1 and node-2)

![Web Dashboard with 2 Nodes](3.png)
*Live dashboard after a second client (node-2) joins. The node comparison chart now shows both nodes side-by-side. node-1 shows OK status while node-2 triggers MEM ALERTs. The packet history table displays interleaved packets from both nodes.*

---

### Client Terminal — node-2

![Client Terminal node-2](4.png)
*Terminal output of node-2 running `client.py` from a separate machine (192.168.43.132), sending encrypted packets to the server at 192.168.43.55:9999. The session ends with a clean `[INFO] Client shutting down.` message.*

---

### Server Terminal — Single Node: Flask Logs and Alert Detection

![Server Terminal with 1 Node](5.png)
*Server terminal during the single-node phase. Flask HTTP request logs for `/api/dashboard` are interleaved with incoming UDP packet rows from node-1. MEM ALERTs are flagged in the packet log each time memory crosses the 70% threshold.*

---

### Server Terminal — Two Nodes: Concurrent Packet Reception

![Server Terminal with 2 Nodes](6.png)
*Server terminal after node-2 connects. Packets from both node-1 (DESKTOP-URIOA2S) and node-2 (poornima) arrive concurrently and are logged in real time, interleaved with Flask dashboard poll requests. node-1 transitions to OK status while node-2 continues triggering MEM ALERTs.*
